...
class PluginUpdateChecker_3_0 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, esc_html__('Check for updates', 'plugin-update-checker'));
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = esc_html__('This plugin is up to date.', 'plugin-update-checker');
			} else if ( $status == 'update_available' ) {
				$message = esc_html__('A new version of this plugin is available.', 'plugin-update-checker');
			} else {

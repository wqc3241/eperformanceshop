...
"use strict";
	...
	function prdctfltr_init_settings() {
		if ( $('.wcpf_mode_presets').length > 0 ) {
			...
			curr.find('span[class^=wcpfs_]').each( function() {
				var curr_el = __get_element($(this));
			...
			curr.find('span[class^=wcpff_]').each( function() {
				var curr_el = __get_element($(this));
				var curr_label = $(this).attr('class');
	...
	$(document).on('click', '.prdctfltr_c_add_filter', function() {
		...
		if ( curr_el.hasClass('pf_active') ) {
			...
			var curr_set = __get_element(curr_form.find('p span[class=wcpfs_'+curr_val+']'));
...
/*		if ( confirm(prdctfltr.localization.remove_override) === false ) {
								...
								if ( curr_preset.wc_settings_prdctfltr_active_filters !== undefined ) {
									...
									$.each(curr_preset.wc_settings_prdctfltr_active_filters, function(index, pf_filter) {
										...
										if ( pf_filter == 'meta' ) {
											...
											var curr_set = __get_element(curr_form.find('p span[class=wcpfs_'+pf_filter+']').parent());
	...
	$(document).ready(function() {
		...
		pf_add_title_disable('full');
	});

	function pf_add_title_disable(mode) {
		$('input[name$="_title"]:not(#wc_settings_prdctfltr_title)').each( function() {
			$(this).prop('disabled', true)
			if ( mode=='full') {
				$(this).after('<input type="checkbox" class="pf-disable-title"'+($(this).val()=='false'?' checked':' ')+' /> Disable title');
			}
		} ) ;
	}
	...
	$(document).on('click', '.pf-disable-title', function() {
		if ( $(this).is(':checked') ) {
			$(this).prev().val('false').prop('disabled', true);
		}
		else {
			$(this).prev().val('').prop('disabled', false);
		}
	} );

	...
	class Azirspares_Attribute_Product_Meta
	{
		...
		public function scripts()
		{
			if ( class_exists( 'WooCommerce' ) ) {
				wp_register_style( 'product-attributes-swatches', $this->plugin_uri . 'woo-attribute.css', array(), '1.0' );
				wp_register_script( 'product-attributes-swatches', $this->plugin_uri . 'woo-attribute.js', array(), '1.0', true );
			}
		}
		...
		public function add_attribute_fields()
		{
            ...
            <div class="form-field" style="display: none;">
                <label><?php esc_html_e( 'Size Box', 'azirspares' ); ?></label>
                <input type="text" name="attribute_size" id="attribute_size" value="30x30">
                <p class="description"><?php esc_html_e( 'Determines Size in this attribute.{width}x{height}', 'azirspares' ); ?></p>
            </div>
		...
		public function edit_attribute_fields()
		{
            ...
            <tr class="form-field form-required" <?php if ( $attribute->attribute_type == 'select' ): ?> style="display: none;" <?php endif; ?>>
                <th scope="row" valign="top"><label><?php _e( 'Size Box', 'azirspares' ); ?></label></th>
                <td>
		...
		public function add_attr_field()
		{
            ...
            <div class="form-field term-display-type-wrap">
                <label for="display_type"><?php _e( 'Display type', 'azirspares' ); ?></label>
                <select name="<?php echo esc_attr( $id_type ); ?>" id="product_attribute_type_id">
                    <option value="-1"><?php echo esc_html__( 'None', 'azirspares' ); ?></option>
                    <option value="color"><?php echo esc_html__( 'Color', 'azirspares' ); ?></option>
                    <option value="photo" selected><?php echo esc_html__( 'Photo', 'azirspares' ); ?></option>
                    <option value="label"><?php echo esc_html__( 'Label', 'azirspares' ); ?></option>
                </select>
                <script type="text/javascript">
            ...
            <div class="form-field term-display-type-wrap swatch photo">
                <label><?php _e( 'Thumbnail', 'azirspares' ); ?></label>
                ...
                <div style="line-height: 60px;">
                    ...
                    <button type="button"
                            class="upload_image_button button"><?php _e( 'Upload/Add image', 'azirspares' ); ?></button>
                    <button type="button"
                            class="remove_image_button button"><?php _e( 'Remove image', 'azirspares' ); ?></button>
                </div>
                <script type="text/javascript">
                    ...
                    jQuery(document).on('click', '.upload_image_button', function (event) {
                        ...
                        file_frame = wp.media.frames.downloadable_file = wp.media({
                            title: '<?php _e( 'Choose an image', 'azirspares' ); ?>',
                            button: {
                                text: '<?php _e( 'Use image', 'azirspares' ); ?>'
                            },
                            multiple: false
                        });
		...
		public function edit_attr_field( $term, $taxonomy )
		{
			...
			$id_color    = "product_attribute_meta[{$this->meta_key}][color]";
			$id_photo    = "product_attribute_meta[{$this->meta_key}][photo]";
			$swatch_term = new Azirspares_Term( $this->meta_key, $term->term_id, $taxonomy, false );
            ...
            <tr class="form-field swatch color <?php echo $swatch_term->get_type() == 'color' ? 'active' : ''; ?>">
                <th scope="row" valign="top"><label><?php _e( '', 'azirspares' ); ?></label></th>
                <td>
		...
		public function product_attr_columns( $columns )
		{
			...
			$new_columns['thumb'] = __( 'Image', 'azirspares' );
		...
		{
			if ( 'thumb' === $column ) {
				$swatch_term = new Azirspares_Term( $this->meta_key, $id, $this->taxonomy, false );
		...
		public function get_product_attribute( $attribute )
		{
			...
			try {
				...
				$product_attribute         = array(
					...
					'slug'         => $this->attribute_taxonomy_name( $attribute->attribute_name ),
					'type'         => $attribute->attribute_type,
					'order_by'     => $attribute->attribute_orderby,
					'has_archives' => (bool)$attribute->attribute_public,
				);
				$product_attribute['size'] = isset( $attribute->attribute_size ) ? $attribute->attribute_size : '30x30';

jQuery(function ($) {
    ...
    var initTab = function () {
        $(document).on('click', '.azirspares-mapper ul.nav li', function (e) {
    ...
    var imageSelector = function () {
        $(document).on('click', '.azirspares-mapper a.btn-add-image, .azirspares-mapper #change-image, .azirspares-mapper a.image-selector', function (event) {
            ...
            if (!window.wr_image_selector) {
                ...
                window.wr_image_selector = wp.media({
                    button: {
                        text: azirspares_mapper.text.img_selector_btn_label,
                    },
                    states: [new wp.media.controller.Library({
                        title: azirspares_mapper.text.img_selector_modal_title,
                ...
                window.wr_image_selector.on('select', function () {
                    ...
                    if (window.wr_image_selector.input_element.attr('id') == 'azirspares_mapper_image') {
                        window.wr_image_selector.input_element.val(attachment.attributes.id);

                        // Update edit area with new image.
                        if ($('.azirspares-mapper-bot > .edit-image').length) {
                            $('.azirspares-mapper-bot .image-wrap > img').attr('src', attachment.attributes.url);
                        } else {
                            var edit_html = $('#azirspares_mapper_image_tmpl').text().replace('%URL%', attachment.attributes.url);

                            $('.azirspares-mapper-bot > .add-image').replaceWith(edit_html);
            ...
            window.wr_image_selector.input_element = $(this).hasClass('image-selector') ? $(this).prev('input') : $('#azirspares_mapper_image');
    ...
    var setupApplication = function () {
        ...
        var PinList = Backbone.Collection.extend({
            ...
            fetch: function () {
                if (window.azirspares_mapper_pins && window.azirspares_mapper_pins.length) {
                    this.add(window.azirspares_mapper_pins);
                }
            },
        });
        ...
        var PinView = Backbone.View.extend({
            tagName: 'div',
            className: 'azirspares-pin',

            // Cache the template function for a single item.
            template: _.template($('#azirspares_mapper_pin_tmpl').text()),
            ...
            render: function () {
                ...
                this.$el.find('[data-option]').each(function () {
                    ...
                    if ($(this).attr('name') != option[0]) {
                        if (option[3] !== undefined) {
                            $(this).attr('name', 'azirspares_mapper_pins[' + self.index + '][' + option[1] + '][' + option[3] + ']');
                        } else {
                            $(this).attr('name', 'azirspares_mapper_pins[' + self.index + '][' + option[1] + ']');
                        }
                    }
                    ...
                    if ($(this).hasClass('color-picker')) {
                        ...
                        setTimeout($.proxy(function () {
                            $(this).parent().parent().click(function () {
                                ...
                                // focused.
                                $('.azirspares-mapper .icon-selector').each(function () {
                    ...
                    // Init icon selector if needed.
                        ...
                        $(this).addClass('hidden').after($('#azirspares_mapper_icon_selector_tmpl').text().replace('%SELECTED%', $(this).val()));
                    }
                    ...
                    // Init product selector if needed.
                        ...
                        $(this).select2({
                            ...
                            ajax: {
                                url: azirspares_mapper.product_selector.url,
                                dataType: 'json',
                                delay: 250,
                                data: function (terms) {
                                    return {
                                        term: terms,
                                        security: azirspares_mapper.product_selector.security,
                                    };
                                },
                            ...
                            initSelection: function (element, callback) {
                                ...
                                if (id !== '') {
                                    $.ajax(azirspares_mapper.product_selector.url + '&term=' + id + '&security=' + azirspares_mapper.product_selector.security, {
                                        dataType: 'json',
                                    }).done(function (data) {
                                        callback(data);
                                    });
                ...
                }).on('click', '.azirspares-icon-list a', function (event) {
                    event.preventDefault();
            ...
            edit: function (event) {
                if (!$(event.target).parent().hasClass('azirspares-pin') || this.just_dragged) {
                    return;
                }
                ...
                if (this.$el.hasClass('editing')) {
                    ...
                    if (!this.$el.data('azirspares_mapper_pin_settings_initialized')) {
                        var self = this;
                        ...
                        this.$el.data('azirspares_mapper_pin_settings_initialized', true);
                    }
            ...
            close: function (event) {
                ...
                if (pin_type == 'woocommerce') {
                    ...
                    this.$el.find('.tooltip').text(title ? title : azirspares_mapper.text.please_input_a_title).removeClass('hidden');
                }
            ...
            remove: function (event) {
                ...
                if (confirm(azirspares_mapper.text.confirm_removing_pin)) {
                    this.$el.remove();
                    this.model.destroy();
        ...
        var PinListView = Backbone.View.extend({
            ...
            el: '.azirspares-mapper-bot > .edit-image > .image-wrap',
            ...
            initialize: function () {
                ...
                $(document).on('click', '.azirspares-mapper-bot > .edit-image > .image-wrap > img', this.create);
            ...
            create: function (event) {
                if (!$('.azirspares-pin.editing').length) {
                    Pins.add([{
        ...
        $(document).on('init_azirspares_mapper', function (event) {
            if (!$(document).data('azirspares_mapper_settings_initialized')) {
                ...
                $('#general-settings').find('input, select, textarea').each(function () {
                    if (window.azirspares_mapper_settings[$(this).attr('name')]) {
                        $(this).val(window.azirspares_mapper_settings[$(this).attr('name')]);

                        if ($(this).attr('type') == 'hidden' && $(this).next().attr('type') == 'checkbox') {
                            if (parseInt(window.azirspares_mapper_settings[$(this).attr('name')])) {
                                $(this).next().attr('checked', 'checked');
                            } else {
                                $(this).next().removeAttr('checked');
                            }
                    ...
                    $(this).attr('name', 'azirspares_mapper_settings[' + $(this).attr('name') + ']');
                ...
                $('#general-settings').on('change', 'input[type="radio"], select', function () {
                    var option = $(this).attr('name').match(/azirspares_mapper_settings\[([^\]]+)\]/), value = $(this).val();
                ...
                $(document).click(function (event) {
                    ...
                    $('.azirspares-mapper .wp-picker-holder .iris-picker').each(function () {
                    ...
                    $('#general-settings.editing, .azirspares-pin.editing').each(function () {
                ...
                // Track data changes.
                var data_fields = '.azirspares-mapper input, .azirspares-mapper select, .azirspares-mapper textarea';

                $(document).on('change', data_fields, function () {
                    if ($(this).attr('name') && $(this).attr('name').indexOf('azirspares_mapper_') > -1) {
                        $('#post #publish').attr('data-changed', 'yes');
                    }
                });
                ...
                $(window).on('beforeunload', function () {
                    ...
                    if (submit_btn.length && submit_btn.attr('data-changed') == 'yes') {
                        return azirspares_mapper.text.ask_for_saving_changes;
                    }
                });

                $(document).data('azirspares_mapper_settings_initialized', true);
            }

            if (!window.azirspares_mapper_pins_app && $('.azirspares-mapper-bot > .edit-image > .image-wrap').length) {
                // Init pin list view.
                window.azirspares_mapper_pins_app = new PinListView;
            }
        });
    };

...
//            if ($multi_csv_import_enabled && is_array($this->file_url)) {
            if (is_array($this->file_url)) {
				...
				//Emorphis:Added to delete not in csv products in monthly cron
				/*else if(count($this->file_url) > 1){
					$GLOBALS['WF_CSV_Product_Import']->delete_products_not_in_csv();
				}*/
				//End Here
                    ...
                    if (is_array($server_csv_files)) {
                        foreach ($server_csv_files as $key => $server_file_name) {
                            ...
                            if (!empty($file_contents)) {
                                file_put_contents(ABSPATH . "wp-content/plugins/product-csv-import-export-for-woocommerce/temp-import".$this->needhi."_$key.csv", $file_contents);
                                $this->error_message = "";
                                $success = true;
                                unset($file_contents);
                            } else {
                ...
                if ($multi_csv_import_enabled) {
                    ...
                    if ($server_csv_files) {
                        ...
                        foreach ($server_csv_files as $key => $server_file1) {
							//Emorphis:Added for path
							$server_file1 = str_replace($ftp_server_path, "", $server_file1);
							//End Here
                            ...
                            if (@ftp_get($ftp_conn, ABSPATH . "wp-content/plugins/product-csv-import-export-for-woocommerce/temp-import".$this->needhi."_$key.csv", $ftp_server_path . '/' . $server_file1, FTP_BINARY)) {
                                $this->error_message = "";
                                $success = true;
                            } else {
                                ftp_pasv($ftp_conn, TRUE); // Enabling passive mode to retry
                                if (@ftp_get($ftp_conn, ABSPATH . "wp-content/plugins/product-csv-import-export-for-woocommerce/temp-import".$this->needhi."_$key.csv", $ftp_server_path . '/' . $server_file1, FTP_BINARY)) { //Retrying after enable passive mode 
                                    $this->error_message = "";
                                    $success = true;
                                } else {
                                    $this->error_message = __("Failed to Download Specified file in FTP Server File Path.<br/><br/><b>Possible Reasons</b><br/><b>1.</b> File path may be invalid.<br/><b>2.</b> Maybe File / Folder Permission missing for specified file or folder in path.<br/><b>3.</b> Write permission may be missing for file <b>plugins/product-csv-import-export-for-woocommerce/temp-import.csv</b>.");
                    ...
                    $server_csv_files = array_map('trim', $server_csv_files);
                    foreach ($server_csv_files as $key => $server_file) {
                    if (@ftp_get($ftp_conn, ABSPATH . "wp-content/plugins/product-csv-import-export-for-woocommerce/temp-import".$this->needhi."_$key.csv", $server_file, FTP_BINARY)) {
                        $this->error_message = "";
                        $success = true;
                    } else {
                        ftp_pasv($ftp_conn, TRUE); // Enabling passive mode to retry
                        if (@ftp_get($ftp_conn, ABSPATH . "wp-content/plugins/product-csv-import-export-for-woocommerce/temp-import".$this->needhi."_$key.csv", $server_file, FTP_BINARY)) { //Retrying after enable passive mode 
                            $this->error_message = "";
                            $success = true;
                        } else {
                              $GLOBALS['WF_CSV_Product_Import']->hf_log_data_change('csv-import', __('Failed to Download Specified file in FTP Server File Path:'. $server_file .'<br/><br/><b>Possible Reasons</b><br/><b>1.</b> File path may be invalid.<br/><b>2.</b> Maybe File / Folder Permission missing for specified file or folder in path.<br/><b>3.</b> Write permission may be missing for file <b>plugins/product-csv-import-export-for-woocommerce/temp-import.csv</b>.' , 'wf_csv_import_export') ."\n");
...
//                            return false;
        ...
        if ($success) {
            ...
            //if ($multi_csv_import_enabled) {
                if ($server_csv_files) {
                    foreach ($server_csv_files as $key => $server_file) {
                        $file = ABSPATH . "wp-content/plugins/product-csv-import-export-for-woocommerce/temp-import".$this->needhi."_$key.csv";
                        if (file_exists($file)) {

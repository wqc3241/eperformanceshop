<?php
/**
 * Famiau Shortcode setup
 *
 * @author   FAMI
 * @category API
 * @package  famiauVcShortcode
 * @since    1.0.0
 */
if ( !class_exists( 'famiauVcShortcode' ) ) {
	class famiauVcShortcode
	{
		/**
		 * Shortcode name.
		 *
		 ...
		 * @var  string
		 */
		protected $css_key = '_famiauVcShortcode_custom_css';
		
		public function __construct()
		{
			if ( !empty( $this->shortcode ) ) {
				add_shortcode( "famiau_{$this->shortcode}", array( $this, 'output_html' ) );
			}
		...
		public function update_post( $post_id )
		{
			...
			if ( $post ) {
				// Generate custom CSS.
				$css = $this->FamiauShortcodesCustomCss( $post->post_content );
				// Update post and save CSS to post meta.
				$this->save_post( $post );
				$this->save_css_postmeta( $post_id, $css );
				do_action( 'famiau_save_post', $post_id );
			} else {
		...
		public function replace_post( $post_id )
		{
			...
			if ( $post ) {
				$post->post_content = preg_replace_callback(
					'/(famiau_custom_id)="[^"]+"/',
					array( $this, 'famiau_shortcode_replace_post_callback' ),
					$post->post_content
				);
			}
		...
		function famiau_shortcode_replace_post_callback( $matches )
		{
			// Generate a random string to use as element ID.
			$id = 'famiau_custom_' . uniqid();
		 ...
		 * @return  string
		 */
		public function FamiauShortcodesCustomCss( $content )
		{
			$css = '';
			...
			if ( preg_match_all( '/' . get_shortcode_regex() . '/', $content, $shortcodes ) ) {
				foreach ( $shortcodes[2] as $index => $tag ) {
					...
					if ( strpos( $tag, 'famiau_' ) !== false ) {
						$class = 'famiauVcShortcode_' . implode( '_', array_map( 'ucfirst', explode( '-', $shortcode ) ) );
				...
				foreach ( $shortcodes[5] as $shortcode_content ) {
					$css .= self::FamiauShortcodesCustomCss( $shortcode_content );
				}
			}
		...
		function famiau_title_shortcode( $title )
		{
			$html = '';
			$html .= '<h3 class="famiau-title">';
			$html .= '<span class="text">' . esc_html( $title ) . '</span>';
			$html .= '</h3>';
			echo apply_filters( 'famiau_custom_title_shortcode', $html, $title );
		}
		...
		{
			extract( $atts );
			$google_fonts_field          = WPBMap::getParam( "famiau_{$this->shortcode}", $key );
		...
		public function add_css_editor( $atts, $tag )
		{
			$css          = '';
			$inner_css    = '';
			$shortcode_id = isset( $atts['famiau_custom_id'] ) ? '.' . $atts['famiau_custom_id'] : '';
			if ( $tag == 'vc_column' || $tag == 'vc_column_inner' ) {
				$inner_css = ' > .vc_column-inner';
			}
			$editor_names = famiauVisualComposer::famiau_responsive_vc_data();
			/* generate main css */
			...
			if ( !empty( $editor_names ) ) {
				...
				foreach ( $editor_names as $key => $data ) {
					...
					if ( isset( $atts["google_fonts_{$key}"] ) )
						$google_fonts_data = famiauVisualComposer::get_google_font_data( $tag, $atts, "google_fonts_{$key}" );

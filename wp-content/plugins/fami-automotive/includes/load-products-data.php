...
function famiau_auto_complete_search_data_via_ajax() {
	$response = array(
		'array'   => '',
		'message' => '',
		'success' => 'no',
	...
	if ( $posts->have_posts() ) { ?>
		...
		<?php while ( $posts->have_posts() ) { ?>
			...
			// Just get simple product
			if ( ! $product || ! $product->is_type( 'famiau' ) ) {
				continue;
			}
			$min_price   = '';
			$max_price   = '';
				...
				if ( is_array( $childs ) && count( $childs ) > 0 ) {
					...
					foreach ( $childs as $child_id ) {
                        ...
                        <div data-product_id="<?php echo esc_attr( $child_id ); ?>"
                             data-min_pice="<?php echo esc_attr( $product_child->get_price() ); ?>"
                             data-max_price="<?php echo esc_attr( $product_child->get_price() ); ?>" <?php post_class( 'product-item product-item-child' ); ?>>
                            <div class="product-inner">
                                <div class="post-thumb">
									<?php
									$image = famiau_resize_image( get_post_thumbnail_id(), null, 60, 60, true, true, false );
									?>
                                ...
                                <div class="product-info">
                                    ...
                                    <span>(<?php echo '#' . $product_child->get_id() . ' - ' . $product_child->get_price_html(); ?>
                                        )</span>
									<?php
            ...
            <div data-product_id="<?php echo esc_attr( get_the_ID() ); ?>"
                 data-min_pice="<?php echo esc_attr( $min_price ); ?>"
                ...
                <div class="product-inner">
                    ...
                    <div class="product-info">
                        ...
                        <span>(<?php echo '#' . get_the_ID() . ' - ' . $product->get_price_html(); ?>)</span>
						<?php
...
add_action( 'wp_ajax_famiau_auto_complete_search_data_via_ajax', 'famiau_auto_complete_search_data_via_ajax' );

...
global $product , $woocommerce , $xoo_cp_ad_rl_no_value , $xoo_cp_ad_rl_ty_value , $xoo_cp_ad_rl_tl_value;


        $type        = $xoo_cp_ad_rl_ty_value;
        $items_count = $xoo_cp_ad_rl_no_value ? $xoo_cp_ad_rl_no_value : 5;
...
if ( $products->have_posts() ) :  ?>
    <?php do_action('xoo_cp_before_suggested_products'); ?>
        ...
        <span class="xcp-rel-head"><?php echo $xoo_cp_ad_rl_tl_value; ?></span>
        <div class="xoo-cp-rel-slider">
            <ul class="xoo-cp-rel-prods">
                <?php while ( $products->have_posts() ) : $products->the_post(); ?>
                    <?php global $product; ?>
                    <li <?php post_class('xoo-cp-rel-sing'); ?>>                
                        <?php do_action( 'xoo_cp_related_products' ); ?>
                    </li>
                <?php endwhile; ?>
            </ul>
        </div>

    <?php do_action('xoo_cp_after_suggested_products'); ?>
<?php endif; ?>

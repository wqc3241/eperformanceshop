...
class Xoo_CP_Core{
	...
	public function __construct(){
		...
		add_action('wc_ajax_xoo_cp_undo_item',array($this,'xoo_cp_undo_item'));
		...
		add_action('wp_ajax_xoo_cp_undo_item',array($this,'xoo_cp_undo_item'));
		add_action('wp_ajax_nopriv_xoo_cp_undo_item',array($this,'xoo_cp_undo_item'));
		add_action('wp_ajax_xoo_cp_empty_cart',array($this,'xoo_cp_empty_cart'));
		add_action('wp_ajax_nopriv_xoo_cp_empty_cart',array($this,'xoo_cp_empty_cart'));
	}
	...
	public function get_notice_html(){
		...
		switch ($this->action) {
			...
			case 'remove':
				$notice = __('Item removed from your cart','added-to-cart-popup-woocommerce').'<span class="xoo-cp-undo" data-xoo_ckey="'.$this->action_ckey.'">Undo?</span>';
				break;
		...
		return '<span class="xoo-cp-icon-check"></span>'.$notice;
	}
	...
	public function set_ajax_fragments($fragments){
		global $xoo_cp_ad_rl_en_value,$xoo_cp_ad_rl_enm_value,$xoo_cp_gl_fullcart_value;

		$cart_count   = WC()->cart->get_cart_contents_count();
		$notice_html  = $this->action ? $this->get_notice_html() : '';
		$notice_class = $this->action ? 'xoo-cp-atcn-active' : '';
		...
		//Notice
		$fragments['div.xoo-cp-atcn'] = '<div class="xoo-cp-atcn '.$notice_class.'">'.$notice_html.'</div>';
	
		//Related products
		if(!$xoo_cp_ad_rl_en_value || (!$xoo_cp_ad_rl_enm_value && wp_is_mobile()) || $this->action == 'update'){

		}
		else{
			$related_products = $this->get_related_products();
			$fragments['div.xoo-cp-rp-container'] = '<div class="xoo-cp-rp-container">'.$related_products.'</div>';
		}

		//Shortcode data
		ob_start();
		wc_get_template('xoo-cp-shortcode.php','','',XOO_CP_PATH.'/templates/');
		$fragments['a.xoo-cp-sc-cont'] = ob_get_clean();
	...
	//Undo
	public function xoo_cp_undo_item(){
		$cart_key = sanitize_text_field($_POST['cart_key']);
		if(!$cart_key) return;
	...
	//Empty cart
	public function xoo_cp_empty_cart(){
		WC()->cart->empty_cart();
		$this->action = 'empty';
		WC_AJAX::get_refreshed_fragments();
	}
	...
	public function get_related_products(){
		...
		ob_start();
		wc_get_template('xoo-cp-related.php',$args,'',XOO_CP_PATH.'/templates/');
		return ob_get_clean();
